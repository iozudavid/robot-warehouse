...
public class PathFindingMultiRobot {
	...
	public ArrayList<Coordinate> aStar() {
		...
		start.setG(0);
		start.setH(start.heuristicsSingleAStar(new Coordinate(goal.xcoord, goal.ycoord),obstacles));
		...
		while (openList.isEmpty()==false) {
			...
			for (SearchCell cell : successors) {
				if (isInClosedList(cell) == true) {
					...
					cell.setH(cell.heuristicsSingleAStar(new Coordinate(goal.xcoord, goal.ycoord),obstacles));
					openList.add(cell);
